...
const selectEmployee = () => {
    return new Promise(function (resolve, reject) {
        ...
        db.query(sql, (err, rows, fields) => {
            ...
            employeesArr = rows.map(row => ({ name: `${row.first_name} ${row.last_name}`, value: row.id }));
            resolve(employeesArr);
        });
    });
};

module.exports = { Employee, createEmployee, updateEmployeeRole, viewAllEmployees, selectEmployee };
